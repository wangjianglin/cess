...
public class EccJwtHelper {
	 ...
	 * by "." characters)
	 */
	public static Jwt decode(String token, ECPublicKey publicKey,ECPrivateKey privateKey) throws BadPaddingException, NoSuchPaddingException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchProviderException, InvalidKeyException, UnsupportedEncodingException {

		byte[] bytes = Ecc.decrypt(Base64.getDecoder().decode(token),privateKey);
...
//		int claimeCount = bytes[3] + 256 * bytes[2];
		...
		EccJwtHeader header = EccJwtHeaderHelper.create(new String(bytes,4,headerCount,"utf-8"));
...
//		crypto = Base64.getDecoder().decode(crypto);
		...
		return new EccJwtImpl(publicKey,header, claims, crypto);
...
//	}
	...
	public static Jwt encode(CharSequence content, ECPublicKey privateKey) throws NoSuchAlgorithmException {
		EccJwtHeader header = EccJwtHeaderHelper.create(Collections.<String, String>emptyMap());
		...
		return new EccJwtImpl(privateKey,header,claims,crypto);
	}
 ...
 */
class EccJwtHeaderHelper {

	static EccJwtHeader create(String header) {
//		byte[] bytes = b64UrlDecode(header);
		byte[] bytes = header.getBytes();
		return new EccJwtHeader(bytes, parseParams(bytes));
	}

	static EccJwtHeader create(Map<String, String> params) {
		Map<String, String> map = new LinkedHashMap<String, String>(params);
		map.put("alg", "ECC");
		EccHeaderParameters p = new EccHeaderParameters(map);
		return new EccJwtHeader(serializeParams(p), p);
	}

	static EccHeaderParameters parseParams(byte[] header) {
		Map<String, String> map = parseMap(utf8Decode(header));
		return new EccHeaderParameters(map);
	}
	...
	private static byte[] serializeParams(EccHeaderParameters params) {
 ...
 *
 */
class EccJwtHeader implements BinaryFormat {
	private final byte[] bytes;

	final EccHeaderParameters parameters;

	/**
	 * @param bytes the decoded header
	 * @param parameters the parameter values contained in the header
	 */
	EccJwtHeader(byte[] bytes, EccHeaderParameters parameters) {
		this.bytes = bytes;
...
class EccHeaderParameters {
	...
	EccHeaderParameters(String alg) {
		this(new LinkedHashMap<String, String>(Collections.singletonMap("alg", alg)));
	}

	EccHeaderParameters(Map<String, String> map) {
...
class EccJwtImpl implements Jwt {
	final EccJwtHeader header;
	 ...
	 */
	EccJwtImpl(ECPublicKey publicKey,EccJwtHeader header, byte[] content, byte[] crypto) {
		this.header = header;
	...
	public byte[] bytes() {
		...
		try {
			return Ecc.encrypt(bs,this.publicKey);
		} catch (Throwable e) {
			e.printStackTrace();
		}
		return null;
