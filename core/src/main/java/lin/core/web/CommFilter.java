...
//		if("/".equals(uri)){
//			request.getRequestDispatcher(resourcePath + uri).forward(request, response);
//			return;
//		}
...
//			request.getRequestDispatcher(resourcePath + uri.substring(0,uri.length()-CONTENT.length())).forward(request, response);
//			return;
//		}
		
//		if(!uri.endsWith(NOT_CONTENT)){
...
//			}
//		}else{
//			uri = uri.substring(0,uri.length()-NOT_CONTENT.length());
//		}
		
		//String version = request.getParameter(VERSION);
		//else{
//		HttpServletRequest r2 = new HttpServletRequestImpl(request,"/test",new HashMap<String,String[]>());
//		//chain.doFilter(r2, new HttpServletResponseImpl(response));
//		chain.doFilter(r2, response);
...
//			tmpJsonParams = new String(tmpJsonParams.getBytes(),"utf-8");
			...
			throw new LinException(-3,"客户端向服务器传输的数据为空！");
		}
