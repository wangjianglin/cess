...
//                }
	...
	private void process(Object object, Method method) throws JSONException {
        ...
        if (object instanceof Class) {
            ...
            this.map((Map<Object,Object>) object, method);
        } else if (object.getClass().isArray()) {
            this.array(object, method);
        } else if (object instanceof Iterable) {
            this.array(((Iterable<Object>) object).iterator(), method);
    ...
    //private static final String resultInto = Properties.getString("result.into.class");
    private static final List<Method> resultIntoMethods;
    static{
		List<Method> methods = new ArrayList<Method>();
//    	try{
//    		String[] rs = resultInto.split(";");
//    		for(String r:rs){
//    			try{
//    				Class<?> c= Class.forName(r);
//    				Method m = c.getMethod("into", Object.class);
//    				if(m != null){
//    					methods.add(m);
//    				}else{
//    					Method[] mds = c.getMethods();
//    					for(Method tm:mds){
//    						if(tm.getAnnotation(IntoValues.class) != null){
//	        					methods.add(tm);
//	        					break;
//    						}
//    					}
...
//    	}
    ...
    private void bean(Object object) throws JSONException {
    	...
    	try{
    		for(Method m:resultIntoMethods){
    			try{
     ...
     */
    private void enumeration(@SuppressWarnings("rawtypes") Enum enumeration) throws JSONException {
    	JsonSerializer js = enumeration.getClass().getAnnotation(JsonSerializer.class);
    	if(js != null){
    		try {
				ValueSerializer serializer = js.serializer().newInstance();
				Object value = serializer.Serialize(enumeration, js.parameter(), java.util.Locale.getDefault());
				this.process(value, serializer.method());
			} catch (Exception e) {
				e.printStackTrace();
			}
    	}else{
	        if (enumAsBean) {
	        	...
	        	//this.string(enumeration.toString());
	        }
    	}
    }
...
//                LOG.debug("Ignoring property because of include rule:  " + expr);
     ...
     * Add map to buffer
     */
    private void map(Map<Object,Object> map, Method method) throws JSONException {
        this.add("{");
    ...
    private void date(Date date, Method method) {
        JSON json = null;
        if(this.formatter==null && this.dateFormatString!=null&&!this.dateFormatString.equals("")){
        	this.formatter = new SimpleDateFormat(this.dateFormatString);
        }
        if (method != null)
