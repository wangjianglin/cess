...
import lin.client.http1.Error;
import lin.client.http1.HttpCommunicateImpl;
import lin.client.http1.HttpPackage;
import lin.client.http1.annotation.HttpPackageReturnType;
...
//	private String contextPath;// = request.getServletContext().getContextPath();
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		if(urlString == null){
			throw new LinException(-0x3_0001,"必须设置url属性");
		}
		try{
			url = new URL(urlString);
		}catch(Throwable e){
			throw new LinException(-0x3_0002,"url属性值无效");
		}
...
//		if(request.getRequestURI().endsWith(".action")){
			HttpCommunicateImpl http = lin.client.http1.HttpCommunicate.get(request.getRequestedSessionId());
...
//					System.out.println("----------------------------");
					...
					try {
//						System.out.println("********************");
						response.getOutputStream().write(lin.util.JsonUtil.serialize(error).getBytes());
					} catch (Exception e) {
						//e.printStackTrace();
					}
				}
